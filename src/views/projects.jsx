...
import {
  PROJECTS, SUBPROJECTS, EXPENSES, VENDORS, PROPERTIES,
  getProperty, getVendor, getClient, fmt, fmtCompact,
} from '../data.js';
...
export function ProjectsViewV2() {
  ...
  const selStyle = { padding: '6px 10px', fontSize: 12, borderRadius: 6, border: '1px solid var(--line)', background: 'var(--bg-elev)', color: 'var(--ink)', cursor: 'pointer', appearance: 'none' };
      ...
      <div className="app-content">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flex: 1, alignItems: 'center', minWidth: 0, flexWrap: 'wrap' }}>
            <select value={filter.status} onChange={e => setFilter({...filter, status: e.target.value})} style={selStyle}>
              {['All','Requested','Scheduled','In progress','On hold','Completed'].map(s => <option key={s} value={s}>{s}</option>)}
            </select>
            <select value={filter.priority} onChange={e => setFilter({...filter, priority: e.target.value})} style={selStyle}>
              {['All','Urgent','High','Medium','Low'].map(s => <option key={s} value={s}>{s}</option>)}
            </select>
            <select value={filter.size} onChange={e => setFilter({...filter, size: e.target.value})} style={selStyle}>
              {['All','Large','Small'].map(s => <option key={s} value={s}>{s}</option>)}
            </select>
            <select value={filter.vendor} onChange={e => setFilter({...filter, vendor: e.target.value})} style={selStyle}>
              <option value="All">All vendors</option>
              {VENDORS.map(v => <option key={v.id} value={v.id}>{v.company}</option>)}
            </select>
            <select value={filter.property} onChange={e => setFilter({...filter, property: e.target.value})} style={selStyle}>
              ...
              {items.map(pr => {
                ...
                return (
                  <div key={pr.id}
                    ...
                    style={{ background: 'var(--bg-elev)', border: '1px solid var(--line)', borderRadius: 10, padding: 12, cursor: 'pointer', transition: 'all 0.15s', boxShadow: 'none' }}
                    ...
                    <div style={{ fontSize: 13, fontWeight: 600, lineHeight: 1.3, marginBottom: 8, color: 'var(--ink)' }}>{pr.title}</div>
    ...
    <div style={{ marginTop: 16 }}>
      <div style={{ background: 'var(--bg-elev)', border: '1px solid var(--line)', borderRadius: 12, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '12px 16px', borderTop: '1px solid var(--line)', fontSize: 10.5, color: 'var(--ink-4)', display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          {[['#6b665f','Requested'],['#2b6fd6','Scheduled'],['#1a1814','In progress'],['#b09444','On hold'],['#1f9d55','Completed']].map(([c,l]) => (
            <span key={l}><span style={{ display: 'inline-block', width: 8, height: 8, background: c, borderRadius: 2, marginRight: 5 }}/>{l}</span>
          ))}
      ...
      <div style={{ background: 'var(--bg-elev)', border: '1px solid var(--line)', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '80px ' + days.map(() => '1fr').join(' '), borderBottom: '1px solid var(--line)', background: 'var(--bg)' }}>
          ...
          {days.map((d, i) => {
            ...
            return (
              <div key={i} style={{ padding: '12px 8px', textAlign: 'center', fontSize: 12, fontWeight: 600, color: isToday ? 'var(--accent)' : 'var(--ink-3)', borderRight: i < 6 ? '1px solid var(--line-2)' : 'none' }}>
                <div style={{ fontSize: 11, color: isToday ? 'var(--accent)' : 'var(--ink-4)' }}>{dayName}</div>
                <div style={{ fontSize: 16, fontWeight: isToday ? 700 : 600, background: isToday ? 'var(--accent)' : 'transparent', color: isToday ? 'white' : 'var(--ink)', width: 28, height: 28, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '4px auto 0' }}>
                  {d.getDate()}
                </div>
              </div>
            );
          ...
          {HOURS.map(h => (
            <div key={h} style={{ display: 'grid', gridTemplateColumns: '80px ' + days.map(() => '1fr').join(' '), minHeight: 60, borderBottom: '1px solid var(--line-2)' }}>
              ...
              {days.map((d, i) => {
                ...
                return (
                  <div key={i} style={{ padding: '4px', borderRight: i < 6 ? '1px solid var(--line-2)' : 'none', display: 'flex', flexDirection: 'column', gap: 2 }}>
                    {cellEvents.map((ev, ei) => (
                      <div key={ei}
                        onClick={() => onOpen(ev.id)}
                        style={{ background: statusColor(ev.status), color: 'white', padding: '4px 6px', borderRadius: 4, fontSize: 9.5, fontWeight: 500, cursor: 'pointer', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', opacity: 0.85, transition: 'opacity 0.15s' }}
                        onMouseEnter={e => e.currentTarget.style.opacity = '1'}
