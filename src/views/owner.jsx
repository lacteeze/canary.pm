...
import { LineChart } from '../ui.jsx';
import { KpiCard, Topbar } from '../shared.jsx';
import { PORTFOLIOS, PROJECTS, getProperty, getClient, fmt, fmtCompact } from '../data.js';
import { PropertiesView, PortfoliosView, ProjectsView } from './manager.jsx';

export function OwnerViews({ section }) {
  const ownerId = 'c1';
  const owner = getClient(ownerId);
  const portfolios = PORTFOLIOS.filter(pf => pf.ownerIds.includes(ownerId));
  const props = [...new Set(portfolios.flatMap(pf => pf.propertyIds))].map(getProperty);
  const totalRent = props.reduce((a, p) => a + p.rent, 0);

  if (section === 'dashboard') {
    return (<>
      <Topbar title={`Welcome back, ${owner.name.split(' ')[0]}`} crumbs="Owner portal"/>
      <div className="app-content">
        <div className="kpi-row">
          <KpiCard label="Monthly distribution (net)" value={fmtCompact(totalRent * 0.82)} delta="+2.1%" up spark={[4.2,4.3,4.4,4.4,4.5,4.5,4.6,4.7]}/>
          <KpiCard label="Properties" value={props.length}/>
          <KpiCard label="Occupancy" value={Math.round(props.filter(p => p.occupancy !== 'Vacant').length / props.length * 100) + '%'}/>
            ...
            <div className="panel-body">
              ...
              <div style={{ marginTop: 16, fontSize: 13 }}>
                {[
                  ...
                  { l: 'Management fee (8%)', v: -totalRent * 0.08, muted: true },
                  { l: 'Maintenance', v: -totalRent * 0.07, muted: true },
                  { l: 'Reserves', v: -totalRent * 0.03, muted: true },
                ].map((r, i) => (
                  <div key={i} style={{ display:'flex', justifyContent:'space-between', padding: '5px 0', borderBottom: '1px solid var(--line-2)' }}>
                    <span style={{ color: r.muted ? 'var(--ink-3)' : 'var(--ink-2)' }}>{r.l}</span>
  ...
  return (<>
    <Topbar title={section === 'statements' ? 'Statements' : 'Documents'} crumbs="Owner portal"/>
    <div className="app-content">
      <div className="panel">
        <table className="data-table">
          <thead><tr><th>Month</th><th>Portfolio</th><th>Gross</th><th>Fees</th><th>Net</th><th></th></tr></thead>
          <tbody>
