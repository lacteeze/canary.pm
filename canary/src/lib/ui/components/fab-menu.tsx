...
function CreateFormModal({ type, onClose }: CreateFormModalProps) {
  const [data, setData] = useState<Record<string, string>>({});

  const formDefs: Record<string, FormDef> = {
    person: {
  ...
  const handleChange = (id: string, value: string) => setData({ ...data, [id]: value });
  const handleSubmit = () => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'flex-end' }}>
      <div style={{ width: '100%', maxWidth: 500, background: 'var(--bg-elev)', borderRadius: '16px 16px 0 0', padding: 24, maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h2 style={{ fontSize: 20, fontWeight: 700, margin: 0 }}>{form.title}</h2>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 24, color: 'var(--ink-3)', padding: 0 }}>✕</button>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {form.fields.map(field => (
            <div key={field.id}>
              <label style={{ display: 'block', fontSize: 12.5, fontWeight: 600, color: 'var(--ink-3)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                {field.label}
              </label>
              {field.type === 'select' ? (
                <select value={data[field.id] || ''} onChange={e => handleChange(field.id, e.target.value)}
                  style={{ width: '100%', padding: '10px 12px', fontSize: 13.5, border: '1px solid var(--line)', borderRadius: 8, background: 'var(--bg)', color: 'var(--ink)', fontFamily: 'inherit' }}>
                  <option value="">Select...</option>
                  {field.options?.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                </select>
              ) : (
                <input type={field.type} placeholder={field.placeholder} value={data[field.id] || ''} onChange={e => handleChange(field.id, e.target.value)}
                  style={{ width: '100%', padding: '10px 12px', fontSize: 13.5, border: '1px solid var(--line)', borderRadius: 8, background: 'var(--bg)', color: 'var(--ink)', fontFamily: 'inherit' }} />
              )}
            </div>
          ))}
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
          ...
          <button onClick={handleSubmit} className="btn btn-primary btn-sm" style={{ flex: 1 }}>Create</button>
        </div>
      </div>
    </div>
  );
...
export default function FabMenu() {
  ...
  return (
    <>
      <button className="fab-button" onClick={() => setOpen(!open)} title="Create new...">+</button>
      {open && (
        <div className="fab-menu">
          {items.map(item => (
            <div key={item.type} className="fab-menu-item" onClick={() => { setFormType(item.type); setOpen(false); }}>
      ...
      {formType && <CreateFormModal type={formType} onClose={() => setFormType(null)} />}
    </>
  );
}
